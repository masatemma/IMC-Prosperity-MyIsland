...
class PastData: 
    def __init__(self):
        ...
        self.portfolio: Dict[str, Tuple[int, int]] = {"AMETHYSTS":(0,0), "STARFRUIT":(0,0)}
        self.prev_mid = -1
        self.mid_prices: Dict[str, List[int]] = {"AMETHYSTS":[], "STARFRUIT":[]}

logger = Logger()

class Trader:
    R_SQUARED_THRESHOLD = {'AMETHYSTS': 1, 'STARFRUIT': 0}
    SIGMA_MULTIPLIER_MAKE = {'AMETHYSTS': 1, 'STARFRUIT': 1.5}
    SIGMA_MULTIPLIER_TAKE = {'AMETHYSTS': 1, 'STARFRUIT': 1}
    # SIGMA_MULTIPLIER_TAKE = {'AMETHYSTS': 1, 'STARFRUIT': 10}
    WINDOW_SIZE_LR = {'AMETHYSTS': 25, 'STARFRUIT': 35}  

    STARFRUIT_PORTFOLIO = True
    ...
    # PORTFOLIO_TRADE_THRESHOLD = {'AMETHYSTS': 10, 'STARFRUIT': 5}
    ...
    def compute_trade_threshold(self, past_trades: PastData, product: str) -> Dict[str, float]:
        ...
        SSE = np.sum([residual**2 for residual in Y - X @ coeffs])
        r_squared = 1 - SSE/SST
    ...
    def compute_amethysts_orders(self, past_trades: PastData, buy_order_depth: Dict[int, int], sell_order_depth: Dict[int, int], product: str) -> List[Order]:
        ...
        if abs(self.positions[product]) < self.POSITION_LIMIT[product] and past_trades.mid_prices[product][-1] >= 10002:
        ...
        elif abs(self.positions[product]) < self.POSITION_LIMIT[product]:
            order_amount = min((self.POSITION_LIMIT[product] + self.positions[product]), market_make_amount)                                        
            orders.append(Order(product, 10002, -order_amount))    
        ...
        if abs(temp_pos) < self.POSITION_LIMIT[product] and past_trades.mid_prices[product][-1] <= 9998:
        ...
        elif abs(temp_pos) < self.POSITION_LIMIT[product]:
            order_amount = min((self.POSITION_LIMIT[product] - temp_pos), market_make_amount)                    
            orders.append(Order(product, 9998, order_amount))
    ...
    def make_threshold_trade(self, trade_threshold, past_trades, buy_order_depth, sell_order_depth, product, trade_portfolio=True):
        ...
        r_squared = trade_threshold['r_squared']
        ...
        """
        if r_squared > self.R_SQUARED_THRESHOLD[product]:
    ...
    def run(self, state: TradingState): 
        ...
        if not state.traderData:
            ...
            past_trades.portfolio = {'AMETHYSTS': (0,0), 'STARFRUIT': (0,0)} # (position, avg_price)
        else:
        ...
        for product in state.listings:
            ...
            if product == "STARFRUIT":
                ...
                if self.cur_timestamp / 100 > self.WINDOW_SIZE_LR[product]-1: 
                    ...
                    portfolio_trade = self.STARFRUIT_PORTFOLIO
